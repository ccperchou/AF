...
def create_connection(user, password, database):
    ...
    config = {
        'user': user,
        'password': password,
        'host': 'localhost',  # ou '127.0.0.1' si MariaDB est en écoute sur localhost
        'database': database,
...
def get_utilisateurs():
    ...
    if connection:
        try:
            ...
            return jsonify({"error": "An error occurred while retrieving utilisateurs"}), 500
    else:
...
def add_utilisateur():
    ...
    if connection:
        try:
            ...
            print(f"Error executing query: {err}")
            return jsonify({"error": "An error occurred while adding utilisateur"}), 500
    else:
        return jsonify({"error": "Failed to connect to the database"}), 500
...
def delete_utilisateur(id):
    ...
    if connection:
        try:
            ...
            print(f"Error executing query: {err}")
            return jsonify({"error": "An error occurred while deleting utilisateur"}), 500
    else:
        return jsonify({"error": "Failed to connect to the database"}), 500
